...
| [langfuse-cursor-hook/](langfuse-cursor-hook/) | **Optional.** Langfuse telemetry + same stack as `docs/hooks.json.example` (`dispatch.sh`, `mcp-guard.sh`). Run `install.sh` locally; never commit `.venv` / `.env`. |
